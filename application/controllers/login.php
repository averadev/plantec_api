...
class Login extends CI_Controller {
	...
	public function checkLogin2(){
        if($this->input->is_ajax_request()){
			$data = $this->admin_user_db->get(array('email' => $_POST['email'], 'contrasena' => md5($_POST['password']), 'catPuestosId' => 3 ));
			//$data2 = $this->admin_user_db->getAdmin(array('email' => $_POST['email'], 'password' => md5($_POST['password'])));
			...
			if(count($data)>0){
				...
				echo json_encode(array('success' => true, 'message' => 'Acceso satisfactorio.', 'type' => 3));
			}else {
				echo json_encode(array('success' => false, 'message' => 'El usuario y/o password es incorrecto.'));
			}
